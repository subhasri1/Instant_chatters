...
//void broadcast_message_online(string name,int joiner_id)
...
int main()
{
	...
	cout<<colors[NUM_COLORS-1]<<"\n\t  ====== Combined Chat of Two users for a session ======   "<<endl<<def_col;
...
/*
//Broadcast online users to the clients accept his ownself
int broadcast_message_online(string all_names, int joiner_id)
{
	char temp[MAX_LEN];
	strcpy(temp,all_names.c_str());
	for(int i=0; i<clients.size(); i++)
	{
		if(clients[i].id!=sender_id)
		{
...
void handle_client(int client_socket, int id)
{
	...
	//broadcast_message_online(name,id);
